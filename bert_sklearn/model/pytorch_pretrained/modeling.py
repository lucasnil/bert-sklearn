...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    logger.info("Loading tf ckpt from  {}".format(tf_path))    
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        tempdir = weights_path = config_path = None
        
        if pretrained_model_name_or_path in PRETRAINED_MODEL_ARCHIVE_MAP:
            model_name = pretrained_model_name_or_path
            archive_file = PRETRAINED_MODEL_ARCHIVE_MAP[model_name]
            
            # extract BioBERT model archive
            if model_name.startswith('biobert'):
                from_tf = True
                _, config_path, weights_path, tempdir = biobert_from_archive(archive_file, cache_dir, logger)
                         
            # extract SciBERT model archive
            elif model_name.startswith('scibert'):
                _, config_path, weights_path, tempdir = scibert_from_archive(archive_file, cache_dir, logger)

            # huggingface s3 model archives: i.e 'bert-base-uncased', etc.
            else:
                config_file = PRETRAINED_CONFIG_ARCHIVE_MAP[model_name]

                config_path, weights_path = bert_from_archive(config_file, archive_file, cache_dir)
        else:
            if from_tf:
                # Directly load from a TensorFlow checkpoint
                weights_path = os.path.join(pretrained_model_name_or_path, TF_WEIGHTS_NAME)
                config_path = os.path.join(pretrained_model_name_or_path, BERT_CONFIG_NAME)
            else:
                weights_path = os.path.join(pretrained_model_name_or_path, WEIGHTS_NAME)
                config_path = os.path.join(pretrained_model_name_or_path, CONFIG_NAME)


        return cls.from_model_ckpt(config_path, weights_path, state_dict,
                                   from_tf=from_tf, tempdir=tempdir, *inputs, **kwargs)

    @classmethod
    def from_model_ckpt(cls, config_file_or_dict, weights_path, state_dict=None,
                        from_tf=False, tempdir=None, *inputs, **kwargs):
        """
        Instantiate a BertPreTrainedModel from a pre-trained model file or a pytorch state dict.
        ...
        Params:
            bert_config: either:
                - a str with the pathname of a json file containg bert model config
                - a dict containg the contents of a bert model config:
                    . `bert_config.json` a configuration file for the model
                    . `pytorch_model.bin` a PyTorch dump of a BertForPreTraining instance
            weights_path:
                - a str with the pathname of a file containing the model weights of either
                - a path a pretrained model archive containing:
                    . a pytorch model checkpoint
            ...
            from_tf: should we load the weights from a locally saved TensorFlow checkpoint
            tempdir: an optional str with the path to a temp dir containing any
                     temporary model files that need to be cleaned up.
            *inputs, **kwargs: additional input for the specific Bert class
                (ex: num_labels for BertForSequenceClassification)
        """
        if isinstance(config_file_or_dict, str):
            config = BertConfig.from_json_file(config_file_or_dict)
        ...
        # If this is a Pytorch checkpoint, load model into state_dict if it has not been passed in        
        ...
        # If this is a TensorFlow checkpoint, load and return model
        if from_tf:
            model = load_tf_weights_in_bert(model, weights_path)
            print("Loading Tensorflow checkpoint from ", weights_path.split("/")[-1])
            if tempdir:
                # Clean up temp dir if we extracted SciBERT or BioBERT archive
                shutil.rmtree(tempdir)
            return model            
...
def bert_from_archive(config_file, archive_file, cache_dir):
    """Download and resolve from cache huggingface S3 archives of BERT models.        
    """
