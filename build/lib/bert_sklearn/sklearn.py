...
class BaseBertEstimator(BaseEstimator):
    ...
    def __init__(self, bert_model='bert-base-uncased',
                 ...
                 num_mlp_hiddens=500, num_mlp_layers=0, restore_file=None, device=None,
        ...
        if restore_file is not None:
            self.restore_finetuned_model(restore_file,device)
    ...
    def restore_finetuned_model(self, restore_file, device=None):
        """
        ...
        if device:
            state = torch.load(restore_file, map_location=torch.device(device))
        else:
...
def load_model(filename, device=None):
    ...
    model = model_ctor(restore_file=filename, device=device)
    return model
